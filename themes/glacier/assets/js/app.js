/*
*	Glacier - Clean & Minimal Portfolio Hugo Theme
*	Version: 1.0.4
...
function mt_portfolio() {
	...
	// Get all portfolio configure
	if (portfolio.length) {
		lazyLoadPortfolio = portfolio.attr('data-lazy-load');
	...
	if (lazyLoadPortfolio == "show") {
		lazyLoadPortfolio = true;
	} else {
		lazyLoadPortfolio = false;
	}

	$('#portfolio').mediaBoxes({
		filterContainer: '.portfolioFilter',
		columns: columns,
		overlayEffect: portfolioStyle,
